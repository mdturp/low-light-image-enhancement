...
function handleFileChange(event) {
  ...
  if (file) {
    ...
    emit('upload', file) // Emit the 'upload' event with the file as payload
  }
}
</script>

<template>
  <div class="flex items-center justify-center w-2/3 sm:w-1/3 h-32">
    <label
      for="dropzone-file"
    ...
    >
      <div class="flex flex-col items-center justify-center pt-5 pb-6">
        ...
        <p class="mb-2 mx-2 text-sm text-gray-500">
          <span class="font-semibold">Click to upload</span> or drag and drop .png, .jpg, .jpeg,
          etc.
        </p>
      </div>
